...
> 延伸阅读：
> - 初始版跨窗同步架构（connectStore 绑定模式）见 [跨窗草稿同步与脏标记仲裁](./跨窗草稿同步与脏标记仲裁.md)。
> - 后续共享原始基线（noteOrigin）+ holders 引用计数、openPreview/openEdit 防覆盖、leavePage keepalive 兜底、独立窗口按钮见 [共享原始基线与防覆盖](./共享原始基线与防覆盖.md)。
