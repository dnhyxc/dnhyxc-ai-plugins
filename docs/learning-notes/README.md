...
| [共享原始基线与防覆盖.md](./共享原始基线与防覆盖.md) | localStorage 共享原始基线（noteOrigin）+ holders 引用计数、openPreview/openEdit 防覆盖（localFromEdit > peer > server）、leavePage keepalive 兜底、独立窗口按钮 |
...
- [共享原始基线与防覆盖.md](./共享原始基线与防覆盖.md) — 共享基线解决各窗 TipTap 漂移；防覆盖优先本地快照
